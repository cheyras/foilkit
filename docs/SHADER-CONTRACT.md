...
| `foil/PenEditor.tsx` + `foil/pen-surface.ts` | The bezier overlay — `MaskEditor`'s sibling, mutually exclusive with it, and deliberately the DUMB half. Every judgement lives in `packages/forge/src/pen-engine.ts` (a pure reducer, `node --test`-driven); this normalises pointer events into `PenInput`, calls `reduce`, and renders `state` — cursor from `cursorFor`, handles from `visibleHandles`, keys from `PEN_KEY_BINDINGS`, and no second table of any of them. TWO LAYERS THAT NEVER MIX: the FILL rasterises through the shared `rasterizePolygons` into the same mask canvas the brush owns, then `maskTexVersion` bumps so the shader preview stays truthful while drawing; the CHROME (anchors, direction lines, rubber band, marquee) renders to an SVG layer above it, so no UI furniture can reach the artifact. Chrome keeps constant SCREEN-pixel size at every zoom — that arithmetic, and the rasterisation, live in `pen-surface.ts` where a test can drive them. A saved raster mask opens as a BACKDROP to trace, never silently vectorised. |
| `foil/CanonLab.tsx` | **Surface A — canon pattern lab** (`/deckscout/foil-lab/canon`, 2026-08-02 split): bare pattern on a blank card (no ink) beside the real reference clip, full 39-slug picker, sliders, tilt, **Save canon** → `data/foil-canon/<patternId>.json` (full uniform snapshot; replaces code defaults as the baseline everywhere). **Card preview** (R4b 2026-08-04): a blank/on-card toggle in the viewer slot renders the live slider state on a RANDOM catalog card the resolver assigns the pattern to, with a re-randomize chip — sampled server-side from the baked inversion `data/foil-pattern-cards.json` (gitignored; regenerate with `node ../../tools/build-pattern-cards.mts` after catalog syncs or resolver changes). Preview honors saved hand masks + adjusted windows (artwork-keyed) but deliberately NOT per-card uniform overrides — the lab edits canon, and a card's sparse override layered over the sliders would misreport what Save canon produces. Patterns with an empty pool show "no catalog cards" and a disabled toggle. **Reference pane** (subtask 12, 2026-09-07): the clip is no longer a file — the corpus cites its footage rather than vendoring it (AGENTS.md F2), so `reference/ReferencePane.tsx` embeds the same seconds of the same source video from `youtube-nocookie.com`, on click, looped by hand (`reference/clipLoop.ts`) because `loop=1` restarts the whole video and `start`/`end` are integer-only. Bounds come from `reference-clips.json` in `@foilkit/patterns`, generated from `reference/<slug>/notes.md`. **Nothing may read pixels out of it** — the iframe is cross-origin; the Gemini passes and the frame-diff harness use local frames from `fetch-reference.sh`. Card adjust carries the same pane behind a closed `<details>`. |
...
| `uInkTex` / `uInkOn` / `uInkDraw` / `uInkTile` / `uInkJitter` / `uInkStagger` / `uInkStrength` / `uInkTone` | **the ink-design layer** (R8-INK 2026-09-07) — the SECOND, orthogonal layer of a reverse holo. The FOIL is embossed micro-texture in aluminium: physical, procedural, one of the 43 types. The DESIGN is opaque ink printed over that sheet, blocking it — an artist's layout from TPCi, which procedural will never converge on because there is nothing to converge to (a dozen visually different reverses share one vertical-sheen sheet). So it is DATA: `data/ink-designs.json`, resolved per printing by `@foilkit/resolver`'s `resolveInk` on `(scope, type, variantKind)` + rarity, resolving card > subset > set > era. It enters `main()` as COVERAGE, where the mask tiers already live — `m *= 1 − uInkStrength·inkCoverage(uv)`, with `uInkTone` optionally lifting the blocked field toward paper. `uInkTex` is ONE lattice CELL (alpha = coverage) placed by `uInkTile` (x = tiles across the card WIDTH, y/z = phase in cells, w = rotation in turns) plus jitter and row stagger; overprints are periodic, so a tile and six numbers scale by construction where a card-sized raster does not. `uInkDraw 0` means the IMAGE ALREADY SHOWS the reverse printing (`data/frames.json` `shows`) and drawing it again would double it — only a MEASURED `reverse` sets it; `unknown` draws. Recipes carrying a procedural stand-in for a printed design (reverse-sheet's ring+dot grid) branch on `uInkOn` and YIELD; drawing both is the double-draw this tier exists to stop. **Surface-owned and STRUCTURAL: never a slider, never in a canon file.** `uInkOn 0` — the default, and every render today — skips every added instruction, which is why this was NOT a composite-contract bump: proven 45/45 byte-identical through `tools/parity`, against a control pair that was itself 45/45. See `docs/INK-DESIGN.md`. |
...
`inkCoverage(uv)` (R8-INK — the ink tile's coverage at a card-space point; guard on
`uInkOn`, and note `main()` has already applied it to `m`, so a recipe reading it is
double-counting),
