...
const TILES = inkIndex.tiles as Record<string, { file: string; across: number; n: number; conf: string | null }>
 ...
 */
const SCOPE_WEIGHT: Record<IndexRow['kind'], number> = { card: 24, subset: 16, set: 8, era: 1 }
...
export function resolveInk(input: {
  ...
  for (const row of ROWS) {
    ...
    if (row.cards && row.kind !== 'card' && row.kind !== 'subset' && !(input.cardId && row.cards.includes(input.cardId)))
      continue
    const score =
      SCOPE_WEIGHT[row.kind] + (row.kinds ? 4 : 0) + (row.types ? 2 : 0) + (row.rar ? 1 : 0)
    hits.push({ row, score })
  }
  ...
  if (row.tile === null) {
    // Queued: the mark is a trademark we may not trace. uInkOn STAYS FALSE, so
    // the recipe keeps its procedural stand-in and the render is exactly what
    // it is today — the slot being empty must cost nothing.
    return {
      tileId: null,
      tileFile: null,
      state: 'queued',
...
export function inkTiles(): Record<string, { file: string; across: number; n: number; conf: string | null }> {
  return TILES
}
