...
re-exports the vector language, `pen-geometry`, the pen engine and its snap
provider, the shared rasteriser and the contour tracer, and nothing that reaches
a builtin.
...
| `pen-engine` | The pen tool's BEHAVIOUR on the same terms: a pure `reduce(state, input, cfg)` state machine with no DOM, no framework and no `node:` builtin, so "feels like Illustrator" is driven by synthetic events in `node --test` rather than only by hand in a browser. Mirrors Illustrator's DOM exactly — absolute handle points, `pointType` as a stored flag — implements the click-precedence ladder as one ordered resolver that `cursorFor` also reads, and enforces its own `maxSnapMovePx` on an injected snap callback rather than trusting it. Ctrl+U (Smart Guides) is the master switch over every snap it performs, `constructionAngles` the opt-in angular capture, and a provider may decline with a stated reason rather than only with silence. |
| `pen-snap` | The provider the engine accepts: `edge-trace`'s structure tensor, prepared once per scan, answering point queries with an existing anchor, a printed-edge corner, the nearest point on a printed edge — or a refusal naming the rival ridges when the scan holds more than one answer. `tools/measure-pen-snap.mts` is where its accuracy and its limits are measured rather than asserted. |
...
239 tests on `node:test`, no dependencies, no build step. They encode what the
